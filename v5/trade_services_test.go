...
func TestPlaceOrderService_Do(t *testing.T) {
	...
	t.Run("empty_data_response", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			w.Header().Set("x-request-id", "rid-place-empty-data")
			w.Header().Set("Content-Type", "application/json")
			_, _ = w.Write([]byte(`{"code":"0","msg":"","data":[]}`))
		}))
		t.Cleanup(srv.Close)
		...
		if got, want := apiErr.RequestID, "rid-place-empty-data"; got != want {
			t.Fatalf("RequestID = %q, want %q", got, want)
		}
		if got, want := apiErr.Code, "0"; got != want {
			t.Fatalf("Code = %q, want %q", got, want)
		}
		if got, want := apiErr.Message, errEmptyPlaceOrderResponse.Error(); got != want {
...
func TestBatchPlaceOrdersService_Do(t *testing.T) {
	...
	t.Run("short_ack_length_mismatch", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			if handleTradeAccountRateLimitMock(w, r) {
				return
			}
			w.Header().Set("x-request-id", "rid-batch-short")
			w.Header().Set("Content-Type", "application/json")
			_, _ = w.Write([]byte(`{"code":"0","msg":"","data":[{"clOrdId":"b15","ordId":"1","tag":"","ts":"1695190491421","sCode":"0","sMsg":""}]}`))
		}))
		t.Cleanup(srv.Close)
		...
		if got, want := batchErr.RequestID, "rid-batch-short"; got != want {
			t.Fatalf("RequestID = %q, want %q", got, want)
		}
...
func TestCancelOrderService_Do_EmptyDataResponse(t *testing.T) {
	fixedNow := time.Date(2020, 3, 28, 12, 21, 41, 274_000_000, time.UTC)
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("x-request-id", "rid-cancel-empty-data")
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"code":"0","msg":"","data":[]}`))
	}))
	t.Cleanup(srv.Close)
	...
	if got, want := apiErr.RequestID, "rid-cancel-empty-data"; got != want {
		t.Fatalf("RequestID = %q, want %q", got, want)
	}
	if got, want := apiErr.Code, "0"; got != want {
		t.Fatalf("Code = %q, want %q", got, want)
	}
	if got, want := apiErr.Message, errEmptyCancelOrderResponse.Error(); got != want {
		t.Fatalf("Message = %q, want %q", got, want)
	}
}

func TestCancelOrderService_Do_MultiAckLengthMismatchFailClose(t *testing.T) {
	fixedNow := time.Date(2020, 3, 28, 12, 21, 41, 274_000_000, time.UTC)
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("x-request-id", "rid-cancel-multi")
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"code":"0","msg":"","data":[{"clOrdId":"","ordId":"1","ts":"0","sCode":"0","sMsg":""},{"clOrdId":"","ordId":"2","ts":"0","sCode":"0","sMsg":""}]}`))
	}))
	t.Cleanup(srv.Close)
	...
	if got, want := apiErr.RequestID, "rid-cancel-multi"; got != want {
		t.Fatalf("RequestID = %q, want %q", got, want)
	}
	if got, want := apiErr.Code, "0"; got != want {
		t.Fatalf("Code = %q, want %q", got, want)
...
func TestCancelOrderService_Do_MultiAckFirstSuccessSecondFailFailClose(t *testing.T) {
	fixedNow := time.Date(2020, 3, 28, 12, 21, 41, 274_000_000, time.UTC)
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("x-request-id", "rid-cancel-multi-fail")
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"code":"0","msg":"","data":[{"clOrdId":"","ordId":"1","ts":"0","sCode":"0","sMsg":""},{"clOrdId":"","ordId":"2","ts":"0","sCode":"70001","sMsg":"Order does not exist."}]}`))
	}))
	t.Cleanup(srv.Close)
	...
	if got, want := apiErr.RequestID, "rid-cancel-multi-fail"; got != want {
		t.Fatalf("RequestID = %q, want %q", got, want)
	}
	if got, want := apiErr.Code, "0"; got != want {
		t.Fatalf("Code = %q, want %q", got, want)
...
func TestBatchCancelOrdersService_Do_ShortAckResponse(t *testing.T) {
	fixedNow := time.Date(2020, 3, 28, 12, 21, 41, 274_000_000, time.UTC)
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("x-request-id", "rid-cancel-batch-short")
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"code":"0","msg":"","data":[{"clOrdId":"","ordId":"1","tag":"","ts":"1695190491421","sCode":"0","sMsg":""}]}`))
	}))
	t.Cleanup(srv.Close)
	...
	if got, want := batchErr.RequestID, "rid-cancel-batch-short"; got != want {
		t.Fatalf("RequestID = %q, want %q", got, want)
	}
	if got, want := batchErr.Expected, 2; got != want {
		t.Fatalf("Expected = %d, want %d", got, want)
...
func TestAmendOrderService_Do_EmptyDataResponse(t *testing.T) {
	fixedNow := time.Date(2020, 3, 28, 12, 21, 41, 274_000_000, time.UTC)
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("x-request-id", "rid-amend-empty-data")
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"code":"0","msg":"","data":[]}`))
	}))
	t.Cleanup(srv.Close)
	...
	if got, want := apiErr.RequestID, "rid-amend-empty-data"; got != want {
		t.Fatalf("RequestID = %q, want %q", got, want)
	}
	if got, want := apiErr.Code, "0"; got != want {
		t.Fatalf("Code = %q, want %q", got, want)
...
func TestBatchAmendOrdersService_Do_ShortAckResponse(t *testing.T) {
	fixedNow := time.Date(2020, 3, 28, 12, 21, 41, 274_000_000, time.UTC)
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("x-request-id", "rid-amend-batch-short")
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"code":"0","msg":"","data":[{"clOrdId":"","ordId":"590909308792049444","reqId":"","ts":"1695190491421","sCode":"0","sMsg":""}]}`))
	}))
	t.Cleanup(srv.Close)
	...
	if got, want := batchErr.RequestID, "rid-amend-batch-short"; got != want {
		t.Fatalf("RequestID = %q, want %q", got, want)
	}
	if got, want := batchErr.Expected, 2; got != want {
		t.Fatalf("Expected = %d, want %d", got, want)
...
func TestGetOrderService_Do(t *testing.T) {
	...
	t.Run("multi_data_fail_close", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.Header().Set("x-request-id", "rid-get-order-multi")
			w.Header().Set("Content-Type", "application/json")
			_, _ = w.Write([]byte(`{"code":"0","msg":"","data":[{"instType":"SPOT","instId":"BTC-USDT","ordId":"1"},{"instType":"SPOT","instId":"BTC-USDT","ordId":"2"}]}`))
		}))
		t.Cleanup(srv.Close)
		...
		_, err := c.NewGetOrderService().InstId("BTC-USDT").OrdId("1").Do(context.Background())
		if err == nil {
			t.Fatalf("expected error")
		}
		apiErr, ok := err.(*APIError)
		...
		if got, want := apiErr.RequestID, "rid-get-order-multi"; got != want {
			t.Fatalf("RequestID = %q, want %q", got, want)
		}
		if got, want := apiErr.Code, "0"; got != want {
			t.Fatalf("Code = %q, want %q", got, want)
		}
		if !strings.Contains(apiErr.Message, "expected 1 item, got 2") {
			t.Fatalf("Message = %q, want contains %q", apiErr.Message, "expected 1 item, got 2")
		}
	})
}
