...
func TestMarketHistoryMarkPriceCandlesService_Do(t *testing.T) {
	t.Run("missing_instId", func(t *testing.T) {
		c := NewClient()
		_, err := c.NewMarketHistoryMarkPriceCandlesService().Do(context.Background())
		if err == nil {
			t.Fatalf("expected error")
		}
		if err != errMarketHistoryMarkPriceCandlesMissingInstId {
			t.Fatalf("error = %v, want %v", err, errMarketHistoryMarkPriceCandlesMissingInstId)
		}
	})
	...
	t.Run("ok", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			if got, want := r.URL.Path, "/api/v5/market/history-mark-price-candles"; got != want {
				t.Fatalf("path = %q, want %q", got, want)
			}
			q := r.URL.Query()
			if got, want := q.Get("instId"), "BTC-USD-240628"; got != want {
		...
		got, err := c.NewMarketHistoryMarkPriceCandlesService().
