...
func TestAssetBillsHistoryService_Do(t *testing.T) {
	...
	t.Run("signed_request", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			if got, want := r.URL.Path, "/api/v5/asset/bills-history"; got != want {
				t.Fatalf("path = %q, want %q", got, want)
			}
			if got, want := r.URL.RawQuery, "after=123&ccy=BTC&clientId=cid&limit=50&pagingType=2&type=1"; got != want {
			...
			if got, want := r.Header.Get("OK-ACCESS-SIGN"), "HTZ9PxzlrnCTG07YeJSZzmceEE80RdmKynG3yncJbRE="; got != want {
		...
		got, err := c.NewAssetBillsHistoryService().
			Ccy("BTC").
			Type("1").
			ClientId("cid").
			After("123").
	...
	t.Run("missing_credentials", func(t *testing.T) {
		...
		_, err := c.NewAssetBillsHistoryService().Do(context.Background())
		if err == nil {
