...
func TestOneClickRepayHistoryService_Do(t *testing.T) {
	fixedNow := time.Date(2020, 3, 28, 12, 21, 41, 274_000_000, time.UTC)

	timestamp := sign.TimestampISO8601Millis(fixedNow)
	wantQuery := "after=1&before=2&limit=10"
	wantPath := "/api/v5/trade/one-click-repay-history?" + wantQuery
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got, want := r.URL.Path, "/api/v5/trade/one-click-repay-history"; got != want {
		...
		_, _ = w.Write([]byte(`{"code":"0","msg":"","data":[{"debtCcy":"USDC","fillDebtSz":"6950.48","fillRepaySz":"4.30","repayCcy":"ETH","status":"filled","uTime":"1661256148746"}]}`))
	}))
	t.Cleanup(srv.Close)
	...
	got, err := c.NewOneClickRepayHistoryService().After("1").Before("2").Limit(10).Do(context.Background())
	if err != nil {
		t.Fatalf("Do() error = %v", err)
	}
	if len(got) != 1 || got[0].DebtCcy != "USDC" || got[0].UTime != 1661256148746 {
		t.Fatalf("data = %#v", got)
	}
}
