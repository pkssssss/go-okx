...
func (s *FinanceStakingDefiRedeemService) Do(ctx context.Context) (*FinanceStakingDefiOrderAck, error) {
	...
	requestID, err := s.c.doWithHeadersAndRequestID(ctx, http.MethodPost, "/api/v5/finance/staking-defi/redeem", nil, s.req, true, nil, &data)
	if err != nil {
		return nil, err
	}
	if len(data) == 0 {
		return nil, newEmptyDataAPIError(http.MethodPost, "/api/v5/finance/staking-defi/redeem", requestID, errEmptyFinanceStakingDefiRedeemAck)
	}
	return &data[0], nil
}
