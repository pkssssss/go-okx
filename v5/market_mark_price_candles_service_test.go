...
func TestMarketMarkPriceCandlesService_Do(t *testing.T) {
	t.Run("missing_instId", func(t *testing.T) {
		c := NewClient()
		_, err := c.NewMarketMarkPriceCandlesService().Do(context.Background())
		if err == nil {
			t.Fatalf("expected error")
		}
		if err != errMarketMarkPriceCandlesMissingInstId {
			t.Fatalf("error = %v, want %v", err, errMarketMarkPriceCandlesMissingInstId)
		}
	})
	...
	t.Run("ok", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			if got, want := r.URL.Path, "/api/v5/market/mark-price-candles"; got != want {
		...
		got, err := c.NewMarketMarkPriceCandlesService().
