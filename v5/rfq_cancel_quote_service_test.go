...
func TestRFQCancelQuoteService_Do(t *testing.T) {
	...
	t.Run("signed_request_and_body", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			if got, want := r.URL.Path, "/api/v5/rfq/cancel-quote"; got != want {
				t.Fatalf("path = %q, want %q", got, want)
			}

			bodyBytes, _ := io.ReadAll(r.Body)
			if got, want := string(bodyBytes), `{"quoteId":"007","clQuoteId":"Bond007"}`; got != want {
			...
			_, _ = w.Write([]byte(`{"code":"0","msg":"","data":[{"quoteId":"007","clQuoteId":"Bond007","sCode":"0","sMsg":""}]}`))
		}))
		t.Cleanup(srv.Close)
		...
		ack, err := c.NewRFQCancelQuoteService().QuoteId("007").ClQuoteId("Bond007").Do(context.Background())
		if err != nil {
			t.Fatalf("Do() error = %v", err)
		}
		if ack == nil || ack.QuoteId != "007" || ack.SCode != "0" {
			t.Fatalf("ack = %#v", ack)
		}
	})
	...
	t.Run("invalid_ack_response", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.Header().Set("Content-Type", "application/json")
			w.Header().Set("X-Request-Id", "rid-rfq-cancel-quote-invalid")
		...
		if got, want := apiErr.RequestID, "rid-rfq-cancel-quote-invalid"; got != want {
			t.Fatalf("apiErr.RequestID = %q, want %q", got, want)
		}
	})
	...
	t.Run("empty_data_response", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.Header().Set("Content-Type", "application/json")
			w.Header().Set("X-Request-Id", "rid-rfq-cancel-quote-empty")
			_, _ = w.Write([]byte(`{"code":"0","msg":"","data":[]}`))
		}))
		t.Cleanup(srv.Close)
		...
		if got, want := apiErr.RequestID, "rid-rfq-cancel-quote-empty"; got != want {
			t.Fatalf("apiErr.RequestID = %q, want %q", got, want)
		}
		if got, want := apiErr.Code, "0"; got != want {
			t.Fatalf("apiErr.Code = %q, want %q", got, want)
		}
		if got, want := apiErr.Message, errEmptyRFQCancelQuote.Error(); got != want {
	...
	t.Run("multi_ack_length_mismatch_fail_close", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.Header().Set("Content-Type", "application/json")
			w.Header().Set("X-Request-Id", "rid-rfq-cancel-quote-multi")
			_, _ = w.Write([]byte(`{"code":"0","msg":"","data":[{"quoteId":"007","clQuoteId":"Bond007","sCode":"0","sMsg":""},{"quoteId":"008","clQuoteId":"Bond008","sCode":"0","sMsg":""}]}`))
		}))
		t.Cleanup(srv.Close)
		...
		if got, want := apiErr.RequestID, "rid-rfq-cancel-quote-multi"; got != want {
			t.Fatalf("apiErr.RequestID = %q, want %q", got, want)
		}
		if got, want := apiErr.Code, "0"; got != want {
			t.Fatalf("apiErr.Code = %q, want %q", got, want)
	...
	t.Run("multi_ack_first_success_second_fail_fail_close", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.Header().Set("Content-Type", "application/json")
			w.Header().Set("X-Request-Id", "rid-rfq-cancel-quote-multi-fail")
			_, _ = w.Write([]byte(`{"code":"0","msg":"","data":[{"quoteId":"007","clQuoteId":"Bond007","sCode":"0","sMsg":""},{"quoteId":"008","clQuoteId":"Bond008","sCode":"70001","sMsg":"Quote does not exist."}]}`))
		}))
		t.Cleanup(srv.Close)
		...
		if got, want := apiErr.RequestID, "rid-rfq-cancel-quote-multi-fail"; got != want {
			t.Fatalf("apiErr.RequestID = %q, want %q", got, want)
		}
		if got, want := apiErr.Code, "0"; got != want {
			t.Fatalf("apiErr.Code = %q, want %q", got, want)
	...
	t.Run("missing_id", func(t *testing.T) {
		...
		if !errors.Is(err, errRFQCancelQuoteMissingId) {
			t.Fatalf("expected errRFQCancelQuoteMissingId, got %T: %v", err, err)
		}
	})
}
