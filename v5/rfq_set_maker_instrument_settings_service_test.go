...
func TestRFQSetMakerInstrumentSettingsService_Do(t *testing.T) {
	...
	t.Run("signed_request_and_body", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			wantBody := `[{"instType":"SPOT","data":[{"instId":"BTC-USDT"},{"instId":"TRX-USDT"}]}]`
		...
		ack, err := c.NewRFQSetMakerInstrumentSettingsService().Settings([]RFQMakerInstrumentSetting{
			{
				InstType: "SPOT",
	...
	t.Run("missing_settings", func(t *testing.T) {
		c := NewClient()
		_, err := c.NewRFQSetMakerInstrumentSettingsService().Do(context.Background())
		if !errors.Is(err, errRFQSetMakerInstrumentSettingsMissingSettings) {
