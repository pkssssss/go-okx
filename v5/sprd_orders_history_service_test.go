...
func TestSprdOrdersHistoryService_Do(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got, want := r.URL.Path, "/api/v5/sprd/orders-history"; got != want {
			t.Fatalf("path = %q, want %q", got, want)
		}
		if got, want := r.URL.RawQuery, "begin=1&beginId=11&end=9&endId=99&limit=10&ordType=limit&sprdId=BTC-USDT_BTC-USDT-SWAP&state=filled"; got != want {
	...
	orders, err := c.NewSprdOrdersHistoryService().
