...
func TestAmendAlgoOrderService_Do(t *testing.T) {
	...
	t.Run("rejects_both_algo_id_and_algo_cl_ord_id", func(t *testing.T) {
		...
		if err != errAmendAlgoOrderTooManyId {
			t.Fatalf("error = %v, want %v", err, errAmendAlgoOrderTooManyId)
		}
	...
	t.Run("multi_ack_length_mismatch_fail_close", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			w.Header().Set("x-request-id", "rid-amend-algo-multi")
			w.Header().Set("Content-Type", "application/json")
			_, _ = w.Write([]byte(`{"code":"0","msg":"","data":[{"algoClOrdId":"algo_01","algoId":"1","reqId":"po","sCode":"0","sMsg":""},{"algoClOrdId":"algo_02","algoId":"2","reqId":"po2","sCode":"0","sMsg":""}]}`))
		}))
		t.Cleanup(srv.Close)
		...
		if got, want := apiErr.RequestID, "rid-amend-algo-multi"; got != want {
			t.Fatalf("RequestID = %q, want %q", got, want)
		}
	...
	t.Run("multi_ack_first_success_second_fail_fail_close", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			w.Header().Set("x-request-id", "rid-amend-algo-multi-fail")
			w.Header().Set("Content-Type", "application/json")
			_, _ = w.Write([]byte(`{"code":"0","msg":"","data":[{"algoClOrdId":"algo_01","algoId":"1","reqId":"po","sCode":"0","sMsg":""},{"algoClOrdId":"algo_02","algoId":"2","reqId":"po2","sCode":"70001","sMsg":"Order does not exist."}]}`))
		}))
		t.Cleanup(srv.Close)
		...
		if got, want := apiErr.RequestID, "rid-amend-algo-multi-fail"; got != want {
			t.Fatalf("RequestID = %q, want %q", got, want)
		}
		if got, want := apiErr.Code, "0"; got != want {
			t.Fatalf("Code = %q, want %q", got, want)
