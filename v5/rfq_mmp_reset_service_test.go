...
func TestRFQMMPResetService_Do(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got, want := r.URL.Path, "/api/v5/rfq/mmp-reset"; got != want {
	...
	ack, err := c.NewRFQMMPResetService().Do(context.Background())
	if err != nil {
		t.Fatalf("Do() error = %v", err)
	}
	if ack == nil || ack.TS != 1597026383085 {
		t.Fatalf("ack = %#v", ack)
	}
}
