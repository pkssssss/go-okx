...
func TestMarketIndexCandlesService_Do(t *testing.T) {
	t.Run("missing_instId", func(t *testing.T) {
		c := NewClient()
		_, err := c.NewMarketIndexCandlesService().Do(context.Background())
		if err == nil {
			t.Fatalf("expected error")
		}
		if err != errMarketIndexCandlesMissingInstId {
			t.Fatalf("error = %v, want %v", err, errMarketIndexCandlesMissingInstId)
		}
	})
	...
	t.Run("ok", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			if got, want := r.URL.Path, "/api/v5/market/index-candles"; got != want {
			...
			_, _ = w.Write([]byte(`{"code":"0","msg":"","data":[["1629966436396","1","2","0.5","1.5","1"],["1629966436397","1","2","0.5","1.5","0"]]}`))
		}))
		t.Cleanup(srv.Close)
		...
		got, err := c.NewMarketIndexCandlesService().
			InstId("BTC-USD").
			Bar("1H").
			After("1000").
			Before("2000").
