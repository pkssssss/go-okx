...
func (s *UsersSubaccountCreateSubaccountService) Do(ctx context.Context) (*UsersSubaccountCreateSubaccountResult, error) {
	...
	if err := s.c.do(ctx, http.MethodPost, "/api/v5/users/subaccount/create-subaccount", nil, req, true, &data); err != nil {
		return nil, err
	}
	if len(data) == 0 {
		return nil, errEmptyUsersSubaccountCreateSubaccountResponse
	}
	return &data[0], nil
}
