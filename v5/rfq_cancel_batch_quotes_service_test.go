...
func TestRFQCancelBatchQuotesService_Do(t *testing.T) {
	...
	t.Run("signed_request_and_partial_failure", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			w.Header().Set("x-request-id", "rid-quote-1")
		...
		if got, want := batchErr.RequestID, "rid-quote-1"; got != want {
			t.Fatalf("RequestID = %q, want %q", got, want)
		}
		if !strings.Contains(err.Error(), "requestId=rid-quote-1") {
	...
	t.Run("empty_data_fail_close", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.Header().Set("x-request-id", "rid-quote-empty")
			w.Header().Set("Content-Type", "application/json")
		...
		acks, err := c.NewRFQCancelBatchQuotesService().QuoteIds([]string{"1150"}).Do(context.Background())
		if err == nil {
