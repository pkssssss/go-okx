...
func TestAccountMMPResetService_Do(t *testing.T) {
	fixedNow := time.Date(2020, 12, 8, 9, 8, 57, 715_000_000, time.UTC)

	t.Run("missing_inst_family", func(t *testing.T) {
		c := NewClient(
			WithNowFunc(func() time.Time { return fixedNow }),
		)
		_, err := c.NewAccountMMPResetService().Do(context.Background())
		if err == nil {
			t.Fatalf("expected error")
		}
		if err != errAccountMMPResetMissingInstFamily {
			t.Fatalf("error = %v, want %v", err, errAccountMMPResetMissingInstFamily)
		}
	})

	t.Run("signed_request_and_body", func(t *testing.T) {
		timestamp := sign.TimestampISO8601Millis(fixedNow)
		wantBody := `{"instType":"OPTION","instFamily":"BTC-USD"}`
		wantSig := sign.SignHMACSHA256Base64("mysecret", sign.PrehashREST(timestamp, http.MethodPost, "/api/v5/account/mmp-reset", wantBody))

		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			if got, want := r.Method, http.MethodPost; got != want {
				t.Fatalf("method = %q, want %q", got, want)
			}
			if got, want := r.URL.Path, "/api/v5/account/mmp-reset"; got != want {
		...
		got, err := c.NewAccountMMPResetService().InstType("OPTION").InstFamily("BTC-USD").Do(context.Background())
		if err != nil {
