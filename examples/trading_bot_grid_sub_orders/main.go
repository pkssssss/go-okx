...
func main() {
	...
	svc := c.NewTradingBotGridSubOrdersService().AlgoId(algoId).AlgoOrdType(algoOrdType).Type(typ)
	if v := os.Getenv("OKX_GROUP_ID"); v != "" {
		svc.GroupId(v)
	}
	...
	fmt.Printf("algoId=%s algoOrdType=%s type=%s count=%d\n", algoId, algoOrdType, typ, len(items))
	if len(items) == 0 {
		return
	}
	it := items[0]
