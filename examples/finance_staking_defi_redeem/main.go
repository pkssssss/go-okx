...
func main() {
	if os.Getenv("OKX_CONFIRM") != "YES" {
		log.Fatal("refusing to redeem staking-defi; set OKX_CONFIRM=YES to continue")
	}
	...
	allowEarlyRedeem := os.Getenv("OKX_ALLOW_EARLY_REDEEM")
	...
	svc := c.NewFinanceStakingDefiRedeemService().
		OrdId(ordId).
		ProtocolType(protocolType)
	if allowEarlyRedeem != "" {
		v, err := strconv.ParseBool(allowEarlyRedeem)
		if err != nil {
			log.Fatal(err)
		}
		svc.AllowEarlyRedeem(v)
	}
