...
func main() {
	...
	svc := okx.NewClient().NewPublicOpenInterestService().InstType(instType)
	...
	it := items[0]
	fmt.Printf("first instId=%s oi=%s oiCcy=%s oiUsd=%s ts=%d\n", it.InstId, it.OI, it.OICcy, it.OIUsd, it.TS)
}
