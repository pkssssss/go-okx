...
func main() {
	...
	svc := c.NewAssetSubaccountBillsService()
	if ccy != "" {
		svc.Ccy(ccy)
	}
	if typ != "" {
	...
	log.Printf("subaccount_bills=%d", len(bills))
	for i := 0; i < len(bills) && i < 5; i++ {
		x := bills[i]
		log.Printf("bill[%d]: billId=%s type=%s subAcct=%s ccy=%s amt=%s ts=%d", i, x.BillId, x.Type, x.SubAcct, x.Ccy, x.Amt, x.TS)
	}
}
