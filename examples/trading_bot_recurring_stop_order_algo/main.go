...
func main() {
	...
	if os.Getenv("OKX_CONFIRM") != "YES" {
		log.Fatal("refusing to stop tradingBot recurring orders; set OKX_CONFIRM=YES to continue")
	}
	...
	acks, err := c.NewTradingBotRecurringStopOrderAlgoService().
		Orders([]okx.TradingBotRecurringStopOrder{{AlgoId: algoId}}).
		Do(context.Background())
	if err != nil {
		log.Fatal(err)
	}

	log.Printf("tradingBot recurring stop-order-algo: acks=%d", len(acks))
	if len(acks) > 0 {
		ack := acks[0]
