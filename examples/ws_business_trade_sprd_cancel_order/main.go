...
func main() {
	...
	if os.Getenv("OKX_CONFIRM") != "YES" {
		log.Fatal("refusing to cancel sprd order via ws; set OKX_CONFIRM=YES to continue")
	}
	...
	ack, err := ws.SprdCancelOrder(opCtx, okx.WSSprdCancelOrderArg{
		OrdId:   ordId,
		ClOrdId: clOrdId,
	})
	if err != nil {
	...
	log.Printf("ws sprd cancel order ack: clOrdId=%s ordId=%s sCode=%s sMsg=%s ts=%d", ack.ClOrdId, ack.OrdId, ack.SCode, ack.SMsg, ack.TS)
}
