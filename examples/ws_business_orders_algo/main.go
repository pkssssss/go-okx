...
func main() {
	...
	ws.OnOrdersAlgo(func(order okx.TradeAlgoOrder) {
		fmt.Printf("channel=%s instType=%s instId=%s algoId=%s ordType=%s state=%s cTime=%d uTime=%d\n",
			okx.WSChannelOrdersAlgo, order.InstType, order.InstId, order.AlgoId, order.OrdType, order.State, order.CTime, order.UTime)
	})
	...
	if err := ws.SubscribeAndWait(subCtx, okx.WSArg{
		Channel:    okx.WSChannelOrdersAlgo,
		InstType:   instType,
	...
	log.Printf("subscribed: channel=%s instType=%s instFamily=%s instId=%s", okx.WSChannelOrdersAlgo, instType, instFamily, instId)
