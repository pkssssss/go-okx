...
func main() {
	if os.Getenv("OKX_CONFIRM") != "YES" {
		log.Fatal("refusing to amend copy settings; set OKX_CONFIRM=YES to continue")
	}
	...
	svc := c.NewCopyTradingAmendCopySettingsService().
		InstType(instType).
	...
	log.Printf("copytrading_amend_copy_settings: result=%v", res.Result)
}
