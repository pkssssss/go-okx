...
- Asset：✅ `asset/currencies`（充提状态/手续费/精度）；✅ `asset/bills`、✅ `asset/bills-history`（资金流水）；✅ `asset/deposit-address`、✅ `asset/deposit-history`；✅ `asset/withdrawal`、✅ `asset/cancel-withdrawal`、✅ `asset/withdrawal-history`；✅ WS `deposit-info`、✅ WS `withdrawal-info`；TODO：子账户相关
