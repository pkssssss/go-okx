...
- 当队列满时，SDK 会通过 `errHandler` 上报 `"queue full; blocking"`，并阻塞等待队列腾出空间（默认不丢弃关键事件）。
- 这保证了“关键事件不丢”，但若 handler 消费不过来，会把背压传递到 read loop，可能导致延迟抖动甚至断线；因此应把 **queue full 视为严重告警信号**。

建议：

- handler 只做轻量分发，把重逻辑交给你自己的 worker；
- 结合 `ws.Stats()` 监控 `TypedQueueLen/Cap`、`RawQueueLen/Cap`，并据此调大 buffer / 拆分 worker / 降载；
- 对关键状态（订单/持仓/余额）请始终保留 REST 对账补偿路径：断线/重连/超时都可能造成数据空窗，需可恢复。
